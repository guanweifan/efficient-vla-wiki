...
- 作者提出 `ThinkProprio`，将 proprioception 以 text token 形式在输入端与 instruction 融合，并用 `instruction + proprio` 共同指导 token selection。论文 headline claim 是：在 `CALVIN`、`LIBERO` 和有限 real-world manipulation 上，`ThinkProprio` 能匹配或超过强 baseline，同时把端到端 inference latency 降低 `50%+`；在 `CALVIN ABC→D` 上 latency 从 `52 ms` 降到 `22 ms`，Avg. Len. 从 `4.44` 提到 `4.55`；在 `LIBERO` 上 average success 从 `96.9%` 到 `97.3%`，并在 `LIBERO-Long` 上达到 `95.2%`。来源：[[raw/2602_06575_ThinkProprio.pdf]]，第 1 页摘要与 Fig. 1；第 5-7 页 Tables 3-8。
