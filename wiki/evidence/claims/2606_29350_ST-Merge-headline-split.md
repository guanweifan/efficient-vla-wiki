# 2606_29350_ST-Merge-headline-split

## 用途
- 当前页服务于 [[wiki/papers/2606_29350_ST-Merge.md|2606_29350_ST-Merge]] 的单篇证据落点，用来拆分新增论文的 headline、主效率瓶颈、secondary relevance 与后续待核 metric。
- 本页是 L1 scan-layer evidence；只承接 title / abstract / method-level 信息，不承担跨论文 synthesis。

## Evidence
- 方法证据命题：Merge redundant visual tokens across space and time during visual encoding using 3D spatio-temporal matching and RoPE-aware positional correction for low-latency robotic VLM / VLA inference. 来源：[[raw/2606_29350_ST-Merge.pdf]]，Title、Abstract、method section。
- 分类证据命题：当前维护分类为 `2.1 Selective Feature Processing`；该判断用于 wiki 路线入口，后续需要用 method details 与实验 setting 继续复核。
