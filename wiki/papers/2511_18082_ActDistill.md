...
- 论文保留了明确的 headline efficiency claim：在 embodied benchmarks 上，ActDistill 能在 **reducing computation by over 50%** 的同时，达到 **up to 1.67× speedup**，并声称在多个设定下取得与 full-scale VLA **comparable or superior** 的表现。这些数字有信息量，但它们分散在不同 backbone 与 benchmark 组合中，例如 OpenVLA / LIBERO 与 CogACT / SIMPLER，不能在 `L1` 被压成单一无条件结论。来源：[[raw/2511_18082_ActDistill.pdf]]，第 1-2 页 Abstract、贡献段；第 6-8 页 Table 1-3、Sec. 4。
- 从当前证据看，ActDistill 更像一篇**action-oriented distillation + adaptive execution** 论文，而不是单纯的 token pruning / early-exit 方法。其新意在于把 teacher 的层级动作语义图显式结构化，并让 router 围绕动作语义而非一般多模态相关性来决定计算路径。来源：[[raw/2511_18082_ActDistill.pdf]]，第 2-5 页 Sec. 3；第 7-8 页 Ablation。
...
- **Table 1**：第 6 页。适合后续补 OpenVLA backbone 在 LIBERO 上的 success / FLOPs / speedup 对比，尤其是 `1.59×` 与 `49.5%` computation 的结果。
- **Table 2**：第 6-7 页。适合后续补 CogACT backbone 在 SIMPLER 的 Visual Matching / Variant Aggregation 结果，以及 `1.67×` 的 speedup。
...
## 待核点
- `over 50% computation reduction`、`up to 1.67× speedup`、`comparable or superior performance` 目前都保留为 headline；后续需要按 `OpenVLA/LIBERO` 与 `CogACT/SIMPLER` 分开拆清。
- 论文在不同地方同时使用 `computation`、`FLOPs`、`speedup`、`latency`，后续需要统一这些效率指标在单篇页中的口径。
...
- 当前结论明显依赖 pretrained teacher；作者也在结尾指出 teacher dependency 是限制之一。后续需要决定这条限制是否应在单篇主 claim 中更显式呈现。
