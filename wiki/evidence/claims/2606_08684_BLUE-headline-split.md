# 2606_08684_BLUE-headline-split

## 用途
- 当前页服务于 [[wiki/papers/2606_08684_BLUE.md|2606_08684_BLUE]] 的单篇证据落点，用来拆分新增论文的 headline、主效率瓶颈、secondary relevance 与后续待核 metric。
- 本页是 L1 scan-layer evidence；只承接 title / abstract / method-level 信息，不承担跨论文 synthesis。

## Evidence
- 方法证据命题：Train a lightweight gate on frozen VLA hidden states to decide per frame whether to generate language or directly predict actions, preserving language benefits at lower inference cost. 来源：[[raw/2606_08684_BLUE.pdf]]，Title、Abstract、method section。
- 分类证据命题：当前维护分类为 `3.2 Reasoning-Aware Action Generation`；次级相关为 `1.2 Dynamic Computation Pathways`；场景标签为 `Autonomous Driving`；该判断用于 wiki 路线入口，后续需要用 method details 与实验 setting 继续复核。
