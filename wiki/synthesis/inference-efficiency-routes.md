...
- `cache-and-reuse`
  ...
  - 依据：通过 temporal、latent、action-state 或 feature/KV reuse 回收已有计算，而不是直接丢弃 token；新近工作把外部 action retrieval、动态区域 token refresh、流式 KV 分区与稀疏历史 feature reuse 纳入这条路线。
