...
- 作者提出 `Discrete Diffusion VLA`，强调两点：一是将 action generation 保持在 unified transformer 内部；二是通过 adaptive decoding order + secondary re-masking 让“先易后难”的并行 refinement 生效。论文 headline 结果包括：`LIBERO` 平均成功率 `96.3%`，`SimplerEnv–Fractal` visual matching `71.2%`、overall `64.1%`，以及 `SimplerEnv–Bridge` overall `54.2%`；此外在 `LIBERO-Goal` 的速度测试中，7B backbone 下单个 action chunk latency 为 `68.8 ms`，约为 AR 的 `2×` 速度。来源：[[raw/2508_20072_DISCRETE-DIFFUSION-VLA.pdf]]，第 1-2 页摘要与引言；第 7-8 页 Tables 1-3；第 11 页 Table 8。
