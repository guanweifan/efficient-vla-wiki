...
5. 做影响分析，判断会影响哪些：
   ...
   - `wiki/index.md`
6. 只更新真正受影响的页面。
7. 同步 `wiki/index.md` 和必要的 `wiki/status.json`；若本地启用 `wiki/log.md`，也要本地补记本轮 `wiki/` 更新，但不要记录外部子模块、下载、extract、release 工具或 Git 历史清理。
...
- 必要的 `evidence/` 更新
- 必要的 `synthesis/` 更新
- 同步后的 `wiki/index.md`、必要的 `wiki/status.json`，以及本地补记的 `wiki/log.md`（若本地启用，且只记录 `wiki/` 更新）
...
- `wiki/index.md`、必要的 `wiki/status.json` 已同步；若本地启用 `wiki/log.md`，本地 wiki 更新日志也已补记
