# 2606_09811_AHA-WAM-headline-split

## 用途
- 当前页服务于 [[wiki/papers/2606_09811_AHA-WAM.md|2606_09811_AHA-WAM]] 的单篇证据落点，用来拆分新增论文的 headline、主效率瓶颈、secondary relevance 与后续待核 metric。
- 本页是 L1 scan-layer evidence；只承接 title / abstract / method-level 信息，不承担跨论文 synthesis。

## Evidence
- 方法证据命题：Decouple WAM inference into a low-frequency video-DiT world planner with reusable rolling KV context and a high-frequency action-DiT executor that routes this context for closed-loop action generation. 来源：[[raw/2606_09811_AHA-WAM.pdf]]，Title、Abstract、method section。
- 分类证据命题：当前维护分类为 `1.3 Dual-system Design`；次级相关为 `2.2 Temporal Sharing and Reuse`；该判断用于 wiki 路线入口，后续需要用 method details 与实验 setting 继续复核。
